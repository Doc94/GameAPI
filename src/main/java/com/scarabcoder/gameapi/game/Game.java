...
public class Game {
	...
	public Game(String id, Arena arena, GameStatus status, Plugin plugin){
		...
		Bukkit.getScheduler().scheduleSyncRepeatingTask(GameAPI.getPlugin(), () -> {
			...
			if(game.isCountingDown()){
				if(game.getCurrentCountdown() < game.getGameSettings().getCountdownTime()){
					...
					if(game.getCurrentCountdown() % 10 == 0){
						game.sendMessage(ChatColor.GREEN + "¡El juego inicia en " + ChatColor.DARK_GREEN + (game.getGameSettings().getCountdownTime() - game.getCurrentCountdown()) + ChatColor.GREEN + " segundos!");
					...
					if(game.isMinimumPlayersFilled()){
						...
						game.sendMessage(ChatColor.RED + "No hay suficientes jugadores para iniciar el juego.");
						game.resetCurrentCountdown();
						game.setCountingDown(false);
					}
				}
	...
	/**
	 * Check if Min of players is get for start the game.
	 * <br>
	 * This ignore SPECTATOR players.
	 * @return TRUE if min is get or false otherwise.
	 */
	public boolean isMinimumPlayersFilled(){
		return this.getPlayers().stream().filter(gamePlayer -> getGamePlayerType(gamePlayer).equals(GamePlayerType.PLAYER)).count() >= this.getGameSettings().getMinimumPlayers();
	}
